...
public class GitRefsFilterConfig {
  ...
  GitRefsFilterConfig(PluginConfigFactory cfgFactory, @Assisted Project.NameKey projectNameKey) {
    try {
      ...
      hideClosedChangesAfterSecs = extractHClosedChangesThreshold(pluginConfig);
  ...
  private static long extractHClosedChangesThreshold(PluginConfig pluginConfig) {
    return Optional.ofNullable(pluginConfig.getString(HIDE_CLOSED_CHANGES_AFTER_FIELD))
        .map(timeString -> ConfigUtil.getTimeUnit(timeString, 0, TimeUnit.SECONDS))
        .orElse(HIDE_CLOSED_CHANGES_AFTER_SECS_DEFAULT);
  }
}
